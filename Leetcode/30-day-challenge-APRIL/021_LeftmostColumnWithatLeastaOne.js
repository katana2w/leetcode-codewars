...
1. (Binary Search) For each row do a binary search to find the leftmost one on that row and update the answer.
   Hide Hint #2
2. (Optimal Approach) Imagine there is a pointer p(x, y) starting from top right corner. p can only move
* left or down. If the value at p is 0, move down. If the value at p is 1, move left. Try to figure out the
...
var leftMostColumnWithOne = function(binaryMatrix) {
    let list = binaryMatrix.dimensions();
    let n = list[0] - 1;
    let m = list[1] - 1;

    let currentP, currN = 0;

    while(n >= currN && m >= 0) {
        if(binaryMatrix.get(currN, m) > 0) {
            currentP = m;
            m--;
        } else {
            currN++;
    ...
    return currentP === undefined ? -1 : currentP;
};